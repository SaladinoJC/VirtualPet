...
public final class ProductSpecifications {
    ...
    public static Specification<ProductEntity> petType(String petType) {
        if (petType == null || petType.isBlank()) {
            return Specification.where(null);
        }
        return (root, query, cb) -> cb.equal(cb.lower(root.get("petType")), petType.toLowerCase());
    }
    ...
    public static Specification<ProductEntity> priceRange(BigDecimal min, BigDecimal max) {
        ...
        return (root, query, cb) -> {
            ...
            if (min != null && max != null) {
                return cb.between(
                        cb.sum(root.get("basePrice"), variants.get("priceDelta")), min, max);
            }
            if (min != null) {
                return cb.greaterThanOrEqualTo(
                        cb.sum(root.get("basePrice"), variants.get("priceDelta")), min);
            }
            return cb.lessThanOrEqualTo(cb.sum(root.get("basePrice"), variants.get("priceDelta")), max);
        };
    }
}
